...
import {
  Clock, Store, Calendar, CheckCircle, XCircle, AlertTriangle, Ban
} from 'lucide-react';
import { format, startOfDay } from 'date-fns';
import type { EmployeeScheduleItem } from '@/types';
...
function AdminScheduleCard({ schedule }: { schedule: EmployeeScheduleItem }) {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <p className="font-semibold text-sm text-slate-900 dark:text-white leading-tight">{schedule.storeName}</p>
...
export function EmployeeScheduleTab({ employeeId }: { employeeId: string }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['employee-schedules', employeeId, page, statusFilter],
    queryFn: () => api.getEmployeeSchedules(employeeId, page, 10, {
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {schedules.map(schedule => <AdminScheduleCard key={schedule.id} schedule={schedule} />)}
        </div>
      )}
