...
import {
  Clock, Store, ChevronDown, ThumbsUp, ThumbsDown, Camera, Package,
...
import type { EmployeeVisitItem } from '@/types';
...
function AdminVisitCard({ visit }: { visit: EmployeeVisitItem }) {
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden hover:border-slate-300 dark:hover:border-slate-600 transition-colors">
      <button className="w-full text-left px-4 py-3.5 flex items-start gap-3" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="w-9 h-9 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center flex-shrink-0 mt-0.5">
          <Store className="w-4 h-4 text-slate-500" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <p className="font-semibold text-sm text-slate-900 dark:text-white leading-tight">{visit.storeName}</p>
            {visitStatus && (
...
export function EmployeeVisitHistoryTab({ employeeId }: { employeeId: string }) {
  const { t, dateLocale } = useTranslation();
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['employee-visits', employeeId, page, statusFilter],
    queryFn: () => api.getEmployeeVisits(employeeId, page, 10, {
  ...
  const groupedVisits = useMemo(() => {
    const groups: Record<string, EmployeeVisitItem[]> = {};
      ...
      {isLoading ? (
        ...
        Object.entries(groupedVisits).map(([dateLabel, groupVisits]) => (
          <div key={dateLabel} className="space-y-2">
            ...
            {groupVisits.map(visit => <AdminVisitCard key={visit.id} visit={visit} />)}
          </div>
        ))
      )}
